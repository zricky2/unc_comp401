...
public interface Picture {
	...
	// Creates a copy of Picture as pixel array picture, mutability 
	// specified by flag.
	
	default public Picture copy(boolean is_mutable) {
		Pixel[][] parray = new Pixel[getWidth()][getHeight()];
		for (int x=0; x<getWidth(); x++) {
			for (int y=0; y<getHeight(); y++) {
				parray[x][y] = getPixel(x,y);
			}
		}

		return (is_mutable) ? 
				new MutablePixelArrayPicture(parray, getCaption()) :
					new ImmutablePixelArrayPicture(parray, getCaption());
				
	}
	
	// Factory method for creating new Picture object specified
	// by dimension and initial color. Mutability preference
	// indicated by flag.

	public static Picture createSolidPicture(int width, int height, Pixel color, boolean is_mutable) {
		if ((width < 0) || (height < 0)) {
		...
		if (is_mutable) {
			return new MutablePixelArrayPicture(parray, "no caption");
		} else {
			return new ImmutablePixelArrayPicture(parray, "no caption");
		}
	}
	
	// Factory method for creating new Picture object from URL
	...
	public static Picture readFromURL(String url) throws IOException {
		...
		for (int x=0; x<bi.getWidth(); x++) {
			for (int y=0; y<bi.getHeight(); y++) {
				parray[x][y] = Pixel.fromRGB(bi.getRGB(x, y));
			}
		}
